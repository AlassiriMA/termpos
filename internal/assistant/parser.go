...
func ProcessNaturalLanguage(input string) (string, error) {
	...
	if strings.HasPrefix(input, "add") || strings.Contains(input, "new product") {
		return handleAddCommand(input)
	} else if strings.HasPrefix(input, "sell") || strings.Contains(input, "purchase") || strings.Contains(input, "buy") {
		return handleSellCommand(input)
	} else if strings.Contains(input, "inventory") || strings.Contains(input, "stock") || strings.Contains(input, "products") {
		if strings.Contains(input, "update") || strings.Contains(input, "change") || strings.Contains(input, "set") {
			return handleUpdateStockCommand(input)
		}
		return handleInventoryCommand(input)
	} else if strings.Contains(input, "report") || strings.Contains(input, "sales") || strings.Contains(input, "revenue") {
		return handleReportCommand(input)
